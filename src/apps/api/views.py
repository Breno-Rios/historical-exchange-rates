...
from .serializers.exchange_rates_serializer import ExchangeRatesSerializer
...
def get_currency_exchange_rates_by_date(request):
    if request.method == 'GET':
        try:
            exchange_date = request.GET.get('exchange-date')

            if not exchange_date:
                raise ValueError('missing exchange_date param')

            exchange_date = DateFormatter.fromisoformat(
                request.GET.get('exchange-date'))

            data = RateService.get_currency_exchange_rates_by_date(
                date=exchange_date)
            content = ExchangeRatesSerializer.list_to_dict(data)
...
def get_currency_exchange_rates_by_currency(request):
    if request.method == 'GET':
        try:
            ...
            currency_code = CurrencyValidator.validate(
                request.GET.get('currency-code'))

            data = RateService.get_currency_exchange_rates_by_currency(
...
def get_currency_exchange_rates_by_data_range(request):
    ...
    if request.method == 'GET':
        try:
            ...
            start_date = DateFormatter.fromisoformat(
                request.GET.get('start-date'))
            end_date = DateFormatter.fromisoformat(request.GET.get('end-date'))

            data = RateService.get_currency_exchange_rates_by_data_range(
